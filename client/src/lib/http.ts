...
export class EntityError extends HttpError {
    ...
    constructor({
        ...
        super({ status, payload, message: 'Lỗi xác thực dữ liệu' })
        this.status = status
        this.payload = payload
    }
}
            ...
            if (isClient) {
                if (!clientLogoutRequest) {
                    clientLogoutRequest = fetch('/api/auth/logout', {
                        method: 'POST',
                        body: null, // Logout sẽ cho phép luôn thành công
                        headers: {
                            ...baseHeaders
                        } as any
                    })
                    try {
                        ...
                        // Chuyển hướng về trang đăng nhập có thể dẫn đến loop vô hạn 
                        // nếu không xử lý đúng
                        // vì rời vào trường hợp tại trang Login, 
                        // chúng ta có gọi đến các API cần accessToken 
                        // Mà accessToken đã bị xóa thì nó lại nhảy vào đây, và cứ thế lặp lại
                        location.href = '/login'
                    }
                }
            } else {
